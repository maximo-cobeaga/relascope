...
A real ReservApp dogfooding run showed the original implementation could finish repository file-walk progress, then continue silently during post-scan graph/import materialization. Interrupting during that silent post-scan window left `doctor` reporting `last_scan: running`.

Follow-up fix applied:

- human-mode scan now reports post-scan stages:
  - `Persisting inventory...`
  - `Marking stale graph facts...`
  - `Materializing graph...`
  - `Materializing shallow imports...`
- minimal graph materialization now reports file/repository progress;
- the scan cancellation token is passed into minimal graph materialization;
- cancellation is checked during minimal graph repository and file loops;
- cancellation is checked after minimal graph materialization before shallow imports start;
- shallow import materialization now reports code-file/import progress;
- the scan cancellation token is passed into shallow import materialization;
- cancellation is checked during shallow import repository, code-file, and import-fact loops;
- cancellation is checked after shallow import materialization before `finish_scan("completed")`;
- deterministic storage coverage proves pre-cancelled minimal graph materialization stops without creating graph facts;
- deterministic storage coverage proves pre-cancelled shallow import materialization stops without creating import edges;
- CLI coverage proves post-scan stage output appears by default and is suppressed by `--silent`.

Follow-up verification from `C:\Users\MAXIMO\Desktop\relascope`:
...
The implementation includes Ctrl+C handling, deterministic tests for persisted `cancelled` status visibility, and follow-up cancellation checkpoints for minimal graph and shallow import materialization. A live manual Ctrl+C test against the large dogfood workspace is still required before archive.
